...
public class OxygenFurnaceCalculator implements Calculator {
    ...
    public StepResult calculate(Map<String, Double> p) {
        ...
        steps.add("1) Приняты данные: масса = " + mass + " кг, расход O₂ = " + oxygenFlow + " м³/ч, T₀ = " + t0 + " °C");

        double k = 2500;
        double Q = k * mass;
        steps.add("2) Энергия нагрева Q = " + Q + " кДж");

        double c = 0.84;
        double deltaT = Q / (mass * c);
        double tFinal = t0 + deltaT;
        steps.add("3) Расчёт конечной температуры T₁ = " + tFinal + " °C");

        double time = mass / oxygenFlow;
        steps.add("4) Время продувки t = " + time + " ч");

        double oxygenUsed = oxygenFlow * time;
        steps.add("5) Общий расход кислорода = " + oxygenUsed + " м³");
